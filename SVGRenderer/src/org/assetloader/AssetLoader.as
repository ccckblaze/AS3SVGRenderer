		...
		protected function startNextLoader() : void
		{
			...
			if(_invoked)
			{
				...
				for(var i : int = 0;i < _numLoaders;i++)
				{
					...
					if(!loader.loaded && !loader.failed && !loader.getParam(Param.ON_DEMAND) && !(loader.getParam(Param.PRIORITY) == Number.NEGATIVE_INFINITY) && !(loader.getParam(Param.BASE_PRIORITY) && loader.getParam(Param.PRIORITY) < 0))
					{
