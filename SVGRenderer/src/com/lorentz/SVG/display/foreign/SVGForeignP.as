package com.lorentz.SVG.display.foreign {
	...
	public class SVGForeignP extends SVGPContainer {				
		public function SVGForeignP(){
			super("p");
		}
			...
			for(var i:int = 0; i < numTextElements; i++){
				...
				if(textElement && textElement is String){		
					if(!drawnText){
						...
						if(parent){						
							drawnText.displayObject.width = getViewPortUserUnit(parent.svgWidth, SVGUtil.WIDTH);
...
//					//						textDirection = drawnText.direction;		
		...
		override public function clone():Object {
			var c:SVGForeignP = super.clone() as SVGForeignP;
			c.svgX = svgX;
			c.svgY = svgY;
