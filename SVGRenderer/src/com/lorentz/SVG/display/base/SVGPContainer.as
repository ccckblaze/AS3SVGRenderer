package com.lorentz.SVG.display.base
{
	...
	public class SVGPContainer extends SVGGraphicsElement
	{
		private var _svgX:String = "0";
		private var _svgY:String = "0";
		private var _textOwner:SVGForeignP;
		public var _renderObjects:Vector.<DisplayObject>;
		private var textSprite:DisplayObject;
		
		public function SVGPContainer(tagName:String) {
			super(tagName);
			
			if(this is SVGForeignP)
				_textOwner = this as SVGForeignP;
		}
		...
		protected function get textOwner():SVGForeignP {
			return _textOwner;
		}
		...
		override protected function setParentElement(value:SVGElement):void {
			...
			if(value is SVGForeignP)
				setTextOwner(value as SVGForeignP);
			else if(value is SVGPContainer)
				setTextOwner((value as SVGPContainer).textOwner);
			else
				setTextOwner(this as SVGForeignP);
		}
		
		private function setTextOwner(value:SVGForeignP):void {
			if(_textOwner != value){
				_textOwner = value;
				
				for each(var element:Object in _textElements){
					if(element is SVGPContainer)
						(element as SVGPContainer).setTextOwner(value);
				}
			}
		}
...
//				text = String.fromCharCode(0x200E) + text + String.fromCharCode(0x200E);
		...
		public function offset(offsetX:Number):void {
			...
			{
				if(renderedText is SVGPContainer){
					var textContainer:SVGPContainer = renderedText as SVGPContainer;
		...
		override public function clone():Object {
			var c:SVGPContainer = super.clone() as SVGPContainer;
